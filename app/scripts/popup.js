...
if (ref === undefined || ref.getAuth() === undefined) {
  ...
  userRef.child('notifications').once('value', function (dataSnapshot) {
    ...
    dataSnapshot.forEach(function (childSnapshot) {
      ...
      $notifCards.append($(document.createElement('div')).addClass('card blue-grey darken-1').append($(document.createElement('div')).addClass('card-content white-text').append($(document.createElement('span')).addClass('card-title').html(childSnapshot.child('title').val())).append($(document.createElement('p')).html(childSnapshot.child('text').val()))).append($(document.createElement('div')).addClass('card-action').append($(document.createElement('a')).html('Dismiss').addClass('dismess-card-action').data('key', childSnapshot.key())).append($(document.createElement('a')).html('Ignore from this app').addClass('ignore-app-card-action').data('key', childSnapshot.key()))));
    });
  });
  userRef.child('ignored_apps').once('value', function (dataSnapshot) {
    ...
    dataSnapshot.forEach(function (childSnapshot) {
      console.log('Notifications for each');
      $ignoredCards.append($(document.createElement('div')).addClass('card blue-grey darken-1').append($(document.createElement('div')).addClass('card-content white-text').append($(document.createElement('span')).addClass('card-title').html(childSnapshot.key()))).append($(document.createElement('div')).addClass('card-action').append($(document.createElement('a')).html('Allow App').addClass('allow-app-card-action').data('key', childSnapshot.key()))));
    });
  });
